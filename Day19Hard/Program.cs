...
namespace Day19Hard
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var count3 = Check(bel, '3');
            var count7 = Check(bel, '7');
            var countC = Check(bel, 'C');
            var countY = Check(bel, 'Y');
            ...
            for(int i=0; front.Count > 0; ++i)
            {
                ...
                {
                    var newString = ForvardReplase(candidat, d.Item1, d.Item2);
                    if (newString.Contains(bel))
                    {
                        Console.WriteLine(i);
                        Console.Read();
                        return;
                    ...
                    foreach(var n in newString)
                    {
                        if (n.Length > bel.Length ||  count3 < Check(n, '3') || count7 < Check(n, '7') || countC < Check(n, 'C') || countY < Check(n, 'Y'))
                            continue;
        ...
        static int Check(string bel, char c)
        {
            int count = 0;
        ...
        static string[] ForvardReplase(string bel, char templ, string repl)
        {
            var res = new List<string>();
            for (int i = 0; i < bel.Length; ++i)
            {
                ...
                res.Add(bel.Substring(0, i) + repl + bel.Substring(i + 1));
            }
